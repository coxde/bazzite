...
tee /usr/lib/systemd/system/flatpak-zen-keepassxc-overrides.service <<EOF
[Unit]
Description=Set Overrides for Flatpaks (Zen and KeePassXC)
ConditionPathExists=!/etc/.%N.stamp
After=local-fs.target

[Service]
Type=oneshot
ExecStart=/usr/libexec/flatpak-zen-keepassxc-overrides.sh
...
tee /usr/libexec/flatpak-zen-keepassxc-overrides.sh <<EOF
#!/usr/bin/bash
...
chmod +x /usr/libexec/flatpak-zen-keepassxc-overrides.sh

# Enable the systemd service
systemctl enable flatpak-zen-keepassxc-overrides.service
